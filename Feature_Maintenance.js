// ■■■ Feature_Maintenance.gs : メンテナンス業務ロジック (修理・耐圧検査) ■■■
...
function getMaintenanceList(mode) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    var note = data[i][5];
      ...
      if (isValidDate(nextInspDate) && nextInspDate <= limitDate) {
...
function submitMaintenance(data) {
  ...
  var staffInfo = getUserInfo(getSafeUserEmail(), userPasscode);
  ...
  if (result && result.success) {
    try {
      ...
      items.forEach(function (item) {
        var rowIndex = preLoadedData.idMap[normalizeId(item.id)];
        var officialId = (rowIndex !== undefined && preLoadedData.data[rowIndex])
          ? preLoadedData.data[rowIndex][0]
          : item.id;
        ...
        moneyLogs.push({
          ...
          action: actionName,
          tankId: formatDisplayId(officialId),
          repairCost: cost,
          repairDetail: detail,
          note: ""
        });
